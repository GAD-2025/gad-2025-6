...
const OnboardingPage = () => {
  ...
  const handleNext = () => {
    if (currentIndex < onBoardingData.length - 1) {
      ...
      // Last step, navigate to home or main app
      navigate('/');
    }
  };
  ...
  return (
    ...
    >
      {/* Removed Status Bar and Header as per user's request */}

      <div
        data-property-1="Variant2"
        onClick={handleNext}
        style={{
          width: 350, // Adjusted width to match other buttons
          height: 42,
          padding: '18px 0', // Adjusted padding for full width button
          left: 20, // Centered
          top: 755,
          position: 'absolute',
          background: 'var(--ivory, #FCF4EC)',
      ...
      >
        ...
        >
          {currentIndex < onBoardingData.length - 1 ? '옆으로' : '시작하기'}
        </div>
      </div>
      ...
      >
        {onBoardingData.map((_, index) => (
          <div
            key={index}
            onClick={() => setCurrentIndex(index)} // Make dot clickable
            style={{
          ...
          >
            <div
              style={{
                ...
                top: 0,
                position: 'absolute',
                background: index === currentIndex ? '#D58699' : '#E3E3E3',
                borderRadius: 24,
              }}
            />
          </div>
      ...
      <img
        style={{ width: 390, height: 270, left: 0, top: 153, position: 'absolute' }}
