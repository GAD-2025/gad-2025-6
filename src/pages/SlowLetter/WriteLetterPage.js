...
import { sendLetter } from '../../api/letter'; // Import the sendLetter API
...
const WriteLetterPage = () => {
  ...
  const handleBackClick = () => {
    navigate('/slow-letter'); // Navigate back to slow letter list page
  };
    ...
    <div style={{width: 390, height: 844, background: 'white', overflow: 'hidden', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'flex-start', display: 'inline-flex'}}>
      <div style={{flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'flex-start', display: 'flex'}}>
        <div style={{width: 390, flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center', gap: 32, display: 'flex'}}>
          ...
          <div data-property-1="button.default" onClick={handleSendLetter} style={{width: 350, paddingLeft: 74, paddingRight: 74, paddingTop: 18, paddingBottom: 18, background: sendButtonBackgroundColor, overflow: 'hidden', borderRadius: '28px !important', justifyContent: 'center', alignItems: 'center', gap: 10, display: 'inline-flex', cursor: isSendButtonActive ? 'pointer' : 'not-allowed'}}>
            <div style={{textAlign: 'center', justifyContent: 'flex-start', display: 'flex', flexDirection: 'column', color: '#F1F1F1', fontSize: 20, fontFamily: 'Pretendard', fontWeight: '700', wordWrap: 'break-word'}}>Send</div>
          </div>
        </div>
        {/* Home Indicator */}
